...
function goTo(id, pushHistory=true) {
  ...
  // toggle desktop sidebars
  const rootWrap = document.getElementById('root-wrap');
  if (rootWrap) {
    if (['splash', 'signup', 'onboarding'].includes(id)) {
      rootWrap.classList.add('hide-desktop-sidebars');
    } else {
      rootWrap.classList.remove('hide-desktop-sidebars');
    }
  }
}
...
function goBack() {
  if (S.history.length > 0) {
    ...
    const rootWrap = document.getElementById('root-wrap');
    if (rootWrap) {
      if (['splash', 'signup', 'onboarding'].includes(dest)) {
        rootWrap.classList.add('hide-desktop-sidebars');
      } else {
        rootWrap.classList.remove('hide-desktop-sidebars');
      }
...
function buildOnboardingTour() {
  ...
  document.getElementById('btn-start-app').onclick = async () => {
    ...
    try {
      const res = await fetch('/api/users', {
        ...
        body: JSON.stringify({ username: S.user.username, teamId: S.user.teamId })
      });
      const data = await res.json();
      if (data.success) S.points = data.user.points;
    } catch (e) {
      S.points = 20;
    }
    buildHome();
    ...
    toast(`+${S.points} pts ✨ Account created bonus!`, 1100);
  };
}
...
function renderWall() {
      ...
      <div class="card-pad">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px;">
          ${["🔴 Rohit's massive SIX!","😱 Dhoni OUT!","🏆 MI Champions!"].map((r,i) => `
            <div style="flex:1 1 100px;min-width:100px;background:#fff;border-radius:var(--r-md);padding:7px;
  ...
  const postsHtml = posts.length ? posts.map(buildPostCard).join('') : `
    <div style="text-align:center; padding: 40px 20px;">
      ...
      <h3 style="color:var(--blue); font-weight:700; margin-bottom:8px; font-size:18px;">It's awfully quiet here...</h3>
      <p style="color:var(--muted); font-size:13px; margin-bottom: 20px;">The crowd is waiting! Be the first to start the chant and show your IPL spirit.</p>
      <button class="btn btn-primary" onclick="document.getElementById('c-inp')?.focus()">Start Cheering 🎉</button>
    </div>
  `;
...
function buildLeaderboard(apiData) {
  ...
  let myRankMsg = `Join FanPulse to see your rank! 🏆`;
  if (S.user && !S.user.guest) {
    const fullList = apiData || LEADERS;
    const me = fullList.find(x => x.username === S.user.username);
    const rNum = me ? me.rank : (fullList.length > 0 ? fullList.length + 1 : 1);
    myRankMsg = `You are <span>#${rNum}</span> this week · ${S.points} pts · Keep going! 💪`;
  }
...
function updateDesktopWidgets() {
  // Populate Top Fans
  const lbPreview = document.getElementById('dsk-leaderboard-preview');
  if (lbPreview) {
    lbPreview.innerHTML = [
      { name: 'Rahul', pts: 1250, team: 'MI' },
      { name: 'Priya', pts: 1120, team: 'CSK' },
      { name: 'Amit', pts: 980, team: 'RCB' }
    ].map((fan, i) => `
      <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);">
        <div style="font-weight:700;color:var(--muted);width:15px;">${i+1}</div>
        <div class="avatar av-xs" style="background:var(--surface-hover);">${ini(fan.name)}</div>
        <div style="flex:1;">
          <div class="t-xs w-700">${fan.name}</div>
          <div class="t-xs c-muted">${fan.team} Fan</div>
        </div>
        <div class="t-xs w-700 c-yellow">${fan.pts}</div>
      </div>
    `).join('') + `<div class="t-xs c-primary" style="text-align:center; padding-top:10px; cursor:pointer;" onclick="navTab('leaderboard')">View Full Leaderboard</div>`;
  }

  // Populate Trending Matches
  const tmContainer = document.getElementById('dsk-trending-matches');
  if (tmContainer) {
    const matches = [
      { id: 'm1', homeTeam: 'MI', awayTeam: 'CSK', venue: 'Wankhede Stadium, Mumbai' },
      { id: 'm2', homeTeam: 'RCB', awayTeam: 'KKR', venue: 'M. Chinnaswamy, Bengaluru' },
      { id: 'm3', homeTeam: 'SRH', awayTeam: 'DC', venue: 'Rajiv Gandhi Int., Hyderabad' }
    ];
    tmContainer.innerHTML = matches.map(m => `
      <div class="match-row" style="padding: var(--sp-2); cursor:pointer;" onclick="S.activeMatchId='${m.id}'; navTab('match')">
        <span style="font-size:18px;">🏏</span>
        <div style="flex:1">
          <div class="w-700" style="font-size:13px;">${m.homeTeam} vs ${m.awayTeam}</div>
          <div class="t-xs c-muted">${m.venue}</div>
        </div>
      </div>
    `).join('') + `<div class="t-xs c-primary" style="text-align:center; padding-top:10px; cursor:pointer;" onclick="navTab('match')">View All Matches</div>`;
  }
}
...
/* ══ BOOT ══════════════════════════════════════════════════ */
// Desktop global navigation listener
document.querySelectorAll('.dsk-nav-item').forEach(el => {
  el.addEventListener('click', (e) => {
    const tab = el.closest('.dsk-nav-item').dataset.tab;
    if (tab) navTab(tab);
  });
});
